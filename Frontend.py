...
NODE_COLORS = ["red", "blue", "purple", "green"]  # Node colors
...
def load_graphs_from_file():
    ...
    try:
        with open(GRAPH_FILE, "r") as f:
            data = json.load(f)
            graphs = {gid: json_graph.node_link_graph(gdata, edges="links") for gid, gdata in data.items()}
...
def draw_graph(graphs, graph_id):
    ...
    if graph_id in graphs:
        ...
        # Draw the graph with optimized parameters
        nx.draw(
            graph,
            pos=pos,
            with_labels=True,
            ...
            for node, (x, y) in pos.items():
                if abs(event.xdata - x) < 0.1 and abs(event.ydata - y) < 0.1:
                    ...
                    if node not in clicked_nodes:
                        ...
                        ax.text(x, y, node, color="black", fontsize=10, ha="center", va="center")
        ...
        def on_motion(event):
            """Handles mouse motion events to drag nodes and cancel edge selection."""
            nonlocal dragged_node, clicked_nodes
            if event.inaxes != ax or dragged_node is None:
                return

            # Cancel edge selection when dragging starts
            if clicked_nodes:
                clicked_nodes = []
